...
class _SearchScreenState extends State<SearchScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final tracks = isSearching
        ? mockTracks.where((t) => t.title.toLowerCase().contains(_query.toLowerCase())).toList()
        : mockTracks;

    final playlists = isSearching
        ? mockPlaylists.where((p) => p.name.toLowerCase().contains(_query.toLowerCase())).toList()
        : mockPlaylists;

    final albums = isSearching
        ? mockAlbums.where((a) => a.title.toLowerCase().contains(_query.toLowerCase())).toList()
        : mockAlbums;
        ...
        body: TabBarView(
          children: [
            ...
            GridView.count(
              ...
              crossAxisSpacing: 8,
              childAspectRatio: 1,
              children: playlists.map((p) => PlaylistCard(playlist: p)).toList(),
            ),
